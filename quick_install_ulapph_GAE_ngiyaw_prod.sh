...
do
	...
	"gae") 		
		...
		echo "Do you want to run pre-check by building it locally first? Warning: it takes sometime! [y/n]"
		flagRunPreCheck=n
		read flagRunPreCheck
		echo "Pre-installation check... executing local go build..."
		if [[ flagRunPreCheck = "y" ]]; then
			cd $HOME/ULAPPH-Cloud-Desktop/
			if [[ $(go build main.go) ]]; then
				echo "failed - cannot deploy yet"
				exit 1
			else
		...
		echo "Overwiting wallpapers..."
		rm $HOME/ULAPPH-Cloud-Desktop/static/img/wallpapers/*
		#Overwrite wallpapers
		cp $HOME/ULAPPH-Cloud-Desktop-WP/ngiyaw/* $HOME/ULAPPH-Cloud-Desktop/static/img/wallpapers/
		#Add static images for website
		cp -R $HOME/ULAPPH-Cloud-Desktop-WP/ngiyaw-images $HOME/ULAPPH-Cloud-Desktop/static/img/
		echo "Overwiting shop static site folder..."
		#Overwrite shop folder
		cp $HOME/ULAPPH-Cloud-Desktop-Websites/ngiyaw/shop/* $HOME/ULAPPH-Cloud-Desktop/static/shop/
		if [ $WATSON_EXPORT == "Y" ]
		then
		...
		echo "Installing index via gcloud: " $PROJECT_ID
		gcloud --project=$PROJECT_ID --account=$EMAIL --verbosity=$VERBOSITY --quiet app deploy index.yaml
		...
		echo "Removing ngiyaw-images..."
		rm -rf $HOME/ULAPPH-Cloud-Desktop/static/img/ngiyaw-images
